...
- If you consult additional sources (wiki pages, GitNexus queries, API docs, code) to inform interview questions, add them to `### Sources consulted` in `## Product Context`
- Set status to `🔄 Interview` while this phase is active
- Do NOT proceed to Phase 3 until the interview loop has completed
- After the final round, explicitly ask the user to confirm they are ready to move to spec definition phase
