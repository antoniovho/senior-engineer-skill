...
- **Run the stack's formatter before committing** — every commit must contain properly formatted code. Discover the specific formatting tool and command for the detected stack via MCP.
- **Format is non-negotiable** — formatting is not a style preference; it is a CI gate. Code that fails format checks will fail CI verification.
- **Format test code too** — test files follow the same formatting rules as production code.
